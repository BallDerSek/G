...
while (true) {
    ...
    while ($claim) {
        ...
        $boundary = '';
        $pa = solveUtils::webkitID($_pa, $boundary);
        ...
        if (!empty($_1) && empty($v['valid'])) {
            ...
            $pe = solveUtils::webkitID($_pe, $boundary);
        } else {
            continue;
        }
        ...
        if (!empty($_2['success']) && !empty($chnl = $_2['channel'])) {
            ...
            $po = solveUtils::webkitID(array_merge($_po, $cap), $boundary);
            
            $end = microtime(true);
            $wait = $chnl['watch_time'] - ($end - $set);
            if ($wait >= 0) styler("waiting $wait", fn() => _sle((int)ceil($wait)));
